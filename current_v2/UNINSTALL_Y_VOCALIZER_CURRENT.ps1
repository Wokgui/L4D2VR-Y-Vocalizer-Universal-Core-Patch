...
Write-Host ""
Write-Host "L4D2VR - Y Vocalizer Shortcut v2.0 - Desinstallation" -ForegroundColor White
Write-Host ""

if (Get-Process -Name "left4dead2" -ErrorAction SilentlyContinue) {
    throw "Ferme Left 4 Dead 2 avant la desinstallation."
}
if (Get-Process -Name "vrserver" -ErrorAction SilentlyContinue) {
    throw "Ferme SteamVR avant la desinstallation."
}
...
if (-not (Test-Path -LiteralPath $configBackup) -or
    ...
    throw "Sauvegarde v2.0 introuvable. Aucun fichier n'a ete modifie."
}
...
Write-Host ""
Write-Host "Configuration L4D2VR originale restauree." -ForegroundColor Green
